 ...
 * bounded object-key wildcard patterns, selected object-field names, or every textual leaf beneath one or
 ...
 *     this level"
 * @param objectKeyPatterns object patterns whose final wildcard selects field names
...
public record JsonExtractionSpec(
    ...
    public JsonExtractionSpec {
        ...
        Objects.requireNonNull(objectKeyPatterns, "objectKeyPatterns must not be null");
        ...
        objectKeyPatterns = objectKeyPatterns.stream().sorted().toList();
        allTextLeavesUnder = allTextLeavesUnder.stream().sorted().toList();
        if (extractsAllTextLeaves
                && (!pointers.isEmpty() || !patterns.isEmpty() || !objectKeyPatterns.isEmpty()
                || !allTextLeavesUnder.isEmpty())) {
        ...
        for (String pattern : objectKeyPatterns) {
            Objects.requireNonNull(pattern, "object key pattern must not be null");
            if (!pattern.startsWith("/") || !pattern.endsWith("/*")) {
                throw new IllegalArgumentException(
                        "JSON object key pattern must start with '/' and end with '/*': " + pattern);
            }
        }
    ...
    public static JsonExtractionSpec allTextLeaves() {
        return new JsonExtractionSpec(true, List.of(), List.of(), List.of(), List.of());
    }

    public static JsonExtractionSpec selectedPointers(Set<String> pointers) {
        Objects.requireNonNull(pointers, "pointers must not be null");
        return new JsonExtractionSpec(false, List.copyOf(pointers), List.of(), List.of(), List.of());
    }
    ...
    public static JsonExtractionSpec selected(Set<String> pointers, Set<String> patterns) {
        ...
        return new JsonExtractionSpec(
                false, List.copyOf(pointers), List.copyOf(patterns), List.of(), List.of());
    }
    ...
    public static JsonExtractionSpec selectedWithSubtrees(
        ...
        Objects.requireNonNull(allTextLeavesUnder, "allTextLeavesUnder must not be null");
        return new JsonExtractionSpec(
                false, List.copyOf(pointers), List.of(), List.of(), List.copyOf(allTextLeavesUnder));
    }

    /** Selects field names beneath explicitly known objects. */
    public static JsonExtractionSpec selectedObjectKeys(Set<String> objectKeyPatterns) {
        Objects.requireNonNull(objectKeyPatterns, "objectKeyPatterns must not be null");
        return new JsonExtractionSpec(
                false, List.of(), List.of(), List.copyOf(objectKeyPatterns), List.of());
    }
    ...
    @Override
    public List<String> objectKeyPatterns() {
        return List.copyOf(objectKeyPatterns);
    }
